...
def dist_pl_pointwise(p0, p1, j):
    dis0 = np.cross(j, p0[..., :3]) - p0[..., 3:6]
    dis0 = np.linalg.norm(dis0, axis=-1)
    dis1 = np.cross(j, p1[..., :3]) - p1[..., 3:6]
    dis1 = np.linalg.norm(dis1, axis=-1)
    dis = dis0 + dis1
    return dis
...
def dist_pl_pointwise_conf(p0, p1, j):
    dist = dist_pl_pointwise(p0, p1, j)
    conf = np.sqrt(p0[..., -1] * p1[..., -1])
    dist = np.sum(dist*conf, axis=-1)/(1e-5 + conf.sum(axis=-1))
    dist[conf.sum(axis=-1)<0.1] = 1e5
    return dist
